...
##############################
############################## Section 2: Calculate differences in days of lactation, time in between donations, and gestational age for each participant (from first to subsequent delivery (t2 - t1)) 
##############################
...
dif <- subset(met, select = c(Participant, Pregnancy, Gestational_age_weeks))
dif2 <- spread(dif, Pregnancy, Gestational_age_weeks)
dif2$Difference_ges <- dif2$subsequent - dif2$first

met$Difference_ges <- dif2$Difference_ges[match(met$Participant, dif2$Participant)]
...
bc2$Difference_ges <- met$Difference_ges[match(bc2$Participant, met$Participant)]
...
regression <- lm(cbind(BC,`2'FL`,`3FL`,DFLac,LNFP1,LNFP2,LNFP3,Fuc,`3'SL`,`6'SL`,LSTb,LSTc,DSLNT,DSLNH,Sia,LNT,LNnT,LNH,DFLNT,FLNH,DFLNH,SUM) ~ Secretor_status + Infant_sex_combinations + Difference_abs + Difference_don + Difference_ges, data = reg)
summary(regression)
...
summary(regression3)
regression3 <- mblm(FDSLNH ~ Difference_ges, data = reg)
summary(regression3)
...
bc3$Difference_ges <- met$Difference_ges[match(bc3$Participant, met$Participant)]
